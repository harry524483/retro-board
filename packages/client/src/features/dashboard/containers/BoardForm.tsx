...
import { Board, Column } from '@retro-board/common';
...
const initialValues: Omit<Board, 'id' | 'createdAt' | 'updatedAt'> = {
  name: '',
  maxVotes: MAX_VOTES,
...
const BoardForm: FC = (): JSX.Element => {
  ...
  const renderedColumns = formik.values.columns.map(({ name }, index) => {
    const columnErrors =
      ...
      (formik.errors.columns[index] as FormikErrors<Column>);
    const columnTouched =
    ...
    return (
      <Input
        ...
        error={columnTouched && columnErrors && columnErrors.name}
      />
    );
  });
